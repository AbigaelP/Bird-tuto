...
public class Player : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        if(Input.GetMouseButtonDown(1) && seedsNumber != 0) { // lors du clique droit
            if (Time.time - lastShotTime >= timeBetweenShots ) { // si le délai entre le dernier lâcher de graine et le moment du clique est assez grand, le joueur peut lâcher la graine
    ...
    public bool HoldSeeds() { // permet de vérifier si le joueur tiens au moins une graine
    ...
    private void OnTriggerEnter2D(Collider2D other)
    {
        ...
        } else if (other.CompareTag("Scoring")) // si le joueur passe entre deux tuyaux opposés, il gagne un point
        {
            ...
            if(!other.gameObject.GetComponent<Graines>().isFalling) { // si le joueur touche une graine qui ne tombe pas, il la garde
                seedsNumber++;
