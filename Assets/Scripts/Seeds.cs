...
public class Graines : MonoBehaviour
{
    ...
    {
        if (!isFalling)
        { // si la graine ne tomber pas, elle se déplace vers la gauche jusqu'à atteindre la gauche de l'écran
        ...
        { // sinon elle se déplace moins vite vers la gauche et tombe jusqu'à être détruite
    ...
    {
        if (isFalling)
        { // on vérifie la collision que si la graine tombe
            if (other.CompareTag("Obstacle"))
            { // si la graine touche un obstacle, elle est détruite 
                Destroy(gameObject);
            }
            else if (other.CompareTag("BabyBird"))
            {  // si graine touche un oisillon, elle est détruite et raport des points au joueur
