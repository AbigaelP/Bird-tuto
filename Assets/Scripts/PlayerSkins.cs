...
public class PlayerSkins : MonoBehaviour
{
    ...
    public Sprite[] sprites; // La liste de sprites correspondant au skin
    ...
    private void Awake() {
        ...
        if(PlayerPrefs.HasKey("PLAYER_SKIN_"+id) && PlayerPrefs.GetInt("PLAYER_SKIN_"+id) == 1) {
            unlocked = true;
    ...
    public void PurchaseOrEquip() {
        if(unlocked) { // permet d'équiper le skin si il a été dévérouillé
                FindObjectOfType<SkinManager>().SetPlayerSkin(this); // on met à jour le skin par défaut des tuyaux
            ...
            if(Coins.score >= price) { // si le skin n'a pas été dévérouillé mais que le joueur a assez de pièces
                ...
                PlayerPrefs.SetInt("PLAYER_SKIN_"+id, 1);
                Coins.SaveScore();
            }
        }
    } 
